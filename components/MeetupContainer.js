  ...
  return (
    <Layout>
      {meetup.node && (
        <article>
          <Head>
            <title>{`Paris.js #${meetup.node.edition} chez ${
              meetup.node.host
            }`}</title>
            <meta
              name="description"
              content={'' /* meetup.node.body.slice(0, 50)*/}
            />
          </Head>
          <div className="container meetupContainer">
            <div className="meetupContainer__title">
              <h1>Paris.js #{meetup.node.edition}</h1>
              {meetup.node.host && <h2>chez {meetup.node.host}</h2>}
              {meetup.node.meetupLink && (
                <a href={meetup.node.meetupLink}>Lien meetup</a>
              )}
            </div>
            <div>
              <ul className="meetupContainer__list">
                {meetup.node.talks &&
                  meetup.node.talks.map(talk => (
...
const BlogPostContainer = withPhenomicApi(Meetup, props => ({
  meetup: query({ path: 'content/meetups', id: props.params.splat })
}))
